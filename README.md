...
Caplet is the M in "MVC". Similar to React, it works independently from any other library, and can easily be used with your existing framework such as React (in fact, it works pretty nicely with it).

Caplet doesn't make any assumptions about your service-level implementation. It just gives organization to your model relationships in a sane way. It also gives you a relatively higher level of encapsulation for your data, while also encouraging you - but not forcefuly - to follow a design a design patterns that scale quite well. 
...
Chances are that you are familiar with 

### Features

- Asynchronous models - make API calls on demand using virtuals (see below).
